...
console.log(userDetails)
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const filteredBookings = user.email
        ? response.data.filter((booking) => booking.user === user.email)
        : response.data;
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/userDetails?id=${user?.email}`, {
      method: 'GET',
      headers: {
  ...
  const columns2 = [
    ...
    { title: 'VehicleName', dataIndex: 'vehicleName', key: 'vehicleName' },
  ];
              ...
              items={[
                {
                  ...
                  children: (
                   <UserPersonalModal personalInfo={userDetails?.personalInformation} email={user?.email}/>
                  ),
                },
                {
                  key: '2',
